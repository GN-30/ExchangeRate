...
const ExpenseTracker = () => {
    ...
    const fetchActiveTrip = async () => {
        ...
        try {
            ...
            console.error(
                'Failed to fetch selected trip:',
                e
            );
        }

        /*
         * If Profile did not provide an active trip,
         * get the latest real database trip.
         */
        if (!loadedTrip) {
            try {
                const token = sessionStorage.getItem('token');
        ...
        /*
         * Session data is only used when it contains
         * a real database trip ID.
         */
        if (!loadedTrip) {
            try {
                const sessionRaw =
    ...
    const fetchExpenses = async (tripId = null) => {
        try {
            ...
            const res = await axios.get(
                `${API_BASE}/expenses?tripId=${encodeURIComponent(
                    activeId
                )}`,
                {
                    headers: {
     ...
     * }
     *
     * If another destination has different values,
     * the graph automatically changes.
     */
     ...
     * ---------------------------------------------------------
     *
     * Convert old "Stay" values to "Accommodation"
     * so planned and actual bars use the same category.
     */
    const actualData = expenses.reduce(
        (acc, curr) => {
            let category =
                curr.category;
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    ₹
                                    {Number(
                                        latestTrip.budget_inr ||
                                            latestTrip.budgetINR ||
                                            0
                                    ).toLocaleString()}
                                </strong>
                            </span>
                            ...
                            >
                                ...
                                >
                                    {latestTrip.currency_code ||
                                        latestTrip.currencyCode ||
                                        'INR'}
                                </strong>
